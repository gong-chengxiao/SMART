...
bool DSM::cas_write_sync(RdmaOpRegion &cas_ror, RdmaOpRegion &write_ror,
    ...
    if (GlobalAddress{cas_ror.dest}.nodeID == GlobalAddress{write_ror.dest}.nodeID) {
        ...
        if (!cas_ror.is_on_chip)
            cas(GlobalAddress{cas_ror.dest}, equal, val, (uint64_t *)cas_ror.source, true, ctx);
        else
            cas_dm(GlobalAddress{cas_ror.dest}, equal, val, (uint64_t *)cas_ror.source, true, ctx);
        if (!write_ror.is_on_chip)
            write((const char *)write_ror.source, GlobalAddress{write_ror.dest}, write_ror.size, true, ctx);
        else
            write_dm((const char *)write_ror.source, GlobalAddress{write_ror.dest}, write_ror.size, true, ctx);
...
std::pair<bool, bool> DSM::two_cas_mask_sync(RdmaOpRegion &cas_ror_1, uint64_t equal_1, uint64_t val_1, uint64_t mask_1,
    ...
    if (GlobalAddress{cas_ror_1.dest}.nodeID == GlobalAddress{cas_ror_2.dest}.nodeID) {
        ...
        if (!cas_ror_1.is_on_chip)
            cas_mask(GlobalAddress{cas_ror_1.dest}, equal_1, val_1, (uint64_t *)cas_ror_1.source, mask_1, true, ctx);
        else
            cas_dm_mask(GlobalAddress{cas_ror_1.dest}, equal_1, val_1, (uint64_t *)cas_ror_1.source, mask_1, true, ctx);
        if (!cas_ror_2.is_on_chip)
            cas_mask(GlobalAddress{cas_ror_2.dest}, equal_2, val_2, (uint64_t *)cas_ror_2.source, mask_2, true, ctx);
        else
            cas_dm_mask(GlobalAddress{cas_ror_2.dest}, equal_2, val_2, (uint64_t *)cas_ror_2.source, mask_2, true, ctx);
