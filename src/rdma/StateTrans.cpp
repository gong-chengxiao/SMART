...
bool modifyDCtoRTS(struct ibv_qp *qp, uint16_t remoteLid, uint8_t *remoteGid,
    ...
    if (ibv_exp_modify_qp(qp, &attr, IBV_EXP_QP_STATE | IBV_EXP_QP_PKEY_INDEX | IBV_EXP_QP_PORT | IBV_EXP_QP_DC_KEY)) {
    ...
    fillAhAttr(&attr.ah_attr, remoteLid, remoteGid, context);
    if (ibv_exp_modify_qp(qp, &attr, IBV_EXP_QP_STATE | IBV_EXP_QP_PATH_MTU | IBV_EXP_QP_AV)) {
        Debug::notifyError("failed to modify QP state to RTR");
        return false;
    }
    ...
    attr.rnr_retry = 7;
    attr.max_rd_atomic = 16;
    if (ibv_exp_modify_qp(qp, &attr, IBV_EXP_QP_STATE | IBV_EXP_QP_TIMEOUT | IBV_EXP_QP_RETRY_CNT | IBV_EXP_QP_RNR_RETRY | IBV_EXP_QP_MAX_QP_RD_ATOMIC)) {
        Debug::notifyError("failed to modify QP state to RTS");
        return false;
    }
